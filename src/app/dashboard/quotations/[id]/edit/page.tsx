...
export default function EditQuotationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Attempt to load from localStorage specifically for this draft ID
      const savedDraft = localStorage.getItem(`quotationDraft_${id}`);
      if (savedDraft) {
        try {
          const parsedDraft = JSON.parse(savedDraft);
          if (parsedDraft.formData) {
            setFormData(parsedDraft.formData);
            loadedCarId = parsedDraft.formData.car;
          }
      ...
      // If no local storage override or invalid, load from the DB returned Quote Data
      if (loadInitial && quoteData && !quoteData.error) {
        setFormData({
            customer: quoteData.customer?._id || '',
            car: quoteData.car?._id || '',
            selectedColor: quoteData.selectedColor || '',
            accessories: quoteData.accessories?.map((a: any) => a._id) || [],
            charges: quoteData.charges || { handling: 0, fastag: 550, extendedWarranty: 0 },
            discounts: quoteData.discounts || { dealer: 0, exchangeBonus: 0, corporate: 0, festival: 0, managerSpecial: 0 },
            exchangeVehicle: quoteData.exchangeVehicle || { brand: '', model: '', year: new Date().getFullYear(), condition: 'Good', expectedValue: 0 }
        });
        if (quoteData.exchangeVehicle && quoteData.exchangeVehicle.brand) {
            setHasExchange(true);
        }
        loadedCarId = quoteData.car?._id || '';
      }
      
      if (loadedCarId) {
         const carDetails = carData.find((c: any) => c._id === loadedCarId);
  ...
  const handleNext = () => {
    ...
    setStep(nextStep);
    localStorage.setItem(`quotationDraft_${id}`, JSON.stringify({ formData, step: nextStep, hasExchange }));
  };
  ...
  const submitQuotation = async (status: string) => {
    ...
    try {
      ...
      localStorage.removeItem(`quotationDraft_${id}`);
